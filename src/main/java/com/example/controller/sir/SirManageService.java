...
public class SirManageService {
	...
	// X6 저장
	public JSONObject saveSir(HashMap<String, Object> param) throws Exception {
		EsRest es = new EsRest();
		JSONObject response = new JSONObject();
		JSONObject paramObj = new JSONObject();
        Date today = new Date();
	...
	// sirList 가져오기 
	public JSONObject getSirList() throws Exception {
		//System.out.println("@@@ getSirList");
		EsRest es = new EsRest();
		JSONObject response = new JSONObject();
		...
		try {
			...
			System.out.println("[e] service "+e);
		}
		return response; 
	}    
