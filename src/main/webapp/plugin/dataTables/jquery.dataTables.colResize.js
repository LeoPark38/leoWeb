 ...
 *
 * For details please refer to: http://www.dataTables
 */
(function( factory ) {
    if ( typeof define === "function" && define.amd ) {
        // amd
        define( ["jquery", "dataTables_resize"], function($) {
        ...
        module.exports = function( root, $ ) {
            ...
            if ( !$ || !$.fn.dataTable ) {
                $ = require( "dataTables_resize" )( root, $ ).$;
            }
        ...
        */
        ...
        buildDom: function() {
            ...
            if (!redraw) {
                ...
                that._table.wrap("<div class='dataTables_scroll " + that.CLASS_TABLE_WRAPPER + "'></div>");
            ...
            if (!redraw && that.options.scrollY) {
				 that._wrapper.height(that.options.scrollY);
//                that.initScroller();
            }
...
//                    that._scrollWrapper.css("right", 0 - that._wrapper.scrollLeft());
        ...
        buildColDoms: function() {
            ...
            for (var i = 0, l = $ths.length; i < l; i++) {
                ...
                if(i===$ths.length-1){
					$col.css({
                    	left: Math.ceil($th.position().left + thWidth-6)
                	});
				}
        ...
        _recalcPositions: function() {
            ...
            for (var i = 0, l = that._tableHeaders.length; i < l; i++) {
                ...
                if(i===l-1){
					 that._columns.eq(i).css("left", pos-6);
				}
            }
            that.calcTableDimensions();
        },
        ...
        checkTableHeight: function() {
            var that = this,
                topMarg = 0, 
                newHeight = that._table.outerHeight() - that._tableBody.innerHeight() - 2;
