			...
			createDateTimePicker = function(input) {
				...
				datetimepicker.setOptions = function(_options) {
					...
					if (options.monthOnly) {
						$(datepicker).find('.xdsoft_month').css('display', 'none');
						timepicker.removeClass('active');
					} else {
						$(datepicker).find('.xdsoft_month').css('display', '');
					}
				...
				var _xdsoft_datetime = new function() {
					...
					_this.apartDate = function(t) {
						var ad;
						var temp = new Date(t);
						//temp.setTime(_this.getCurrentTime());
						...
						return temp.dateFormat(options.format);
					};

					_this.getWeek = function(dt) {
					...
					_this.strtodatetime = function(sDateTime) {
						if (options.usePartition) {
							sDateTime.split("~");
							sDateTime = sDateTime.split("~")[0].trim();
						}
				...
				datetimepicker
					.on('xchange.xdsoft', function(event) {
						var table = '';
						var start = new Date(_xdsoft_datetime.currentTime.getFullYear(), _xdsoft_datetime.currentTime.getMonth(), 1),
							temp = new Date(_xdsoft_datetime.currentTime.getFullYear(), _xdsoft_datetime.currentTime.getMonth(), 1), i = 0,
						...
						if (options.monthOnly) {
							...
							if (options.usePartition) {
								...
								for (var k = 0; k < 4; k++) {
									...
									var qua = (3 * k);
									table += '<td data-date="1" data-month="' + qua + '" data-year="' + start.getFullYear() + '">' + options.i18n[options.lang].quarter[k] + '</td>';
									// month가 아닌거 같은데..
									table += '</tr>';
								}
							} else {
								for (var k = 0; k < 4; k++) {
									...
									for (var j = 0; j < 3; j++) {
										var mon = (k * 3) + j;
										table += '<td data-date="1" data-month="' + mon + '" data-year="' + start.getFullYear() + '">' + options.i18n[options.lang].months[mon].substr(0, 3) + '</td>';
									}
									table += '</tr>';
								}
							}
							...
							if (options.usePartition) {
								...
								while (i < _xdsoft_datetime.currentTime.getDaysInMonth() || start.getDay() != options.dayOfWeekStart || _xdsoft_datetime.currentTime.getMonth() == start.getMonth()) {
									...
									if (start.getDay() == 0 || ~options.weekends.indexOf(start.dateFormat('d.m.Y'))) {
										var zero_m = "";
										var zero_d = "";
										...
										if (temp.getMonth() + 1 < 10) zero_m = "0";
										if (temp.getDate() < 10) zero_d = "0";

										ed = zero_m + (temp.getMonth() + 1) + "/" + zero_d + temp.getDate();
										var weeks = _xdsoft_datetime.getWeek(start);
										if (start < _xdsoft_datetime.currentTime && _xdsoft_datetime.currentTime < temp) {
											classes.push('xdsoft_current');
										}

										table += '<td data-date="' + d + '" data-month="' + m + '" data-year="' + y + '"' + ' class="xdsoft_date xdsoft_day_of_week' + start.getDay() + ' ' + classes.join(' ') + '">' + '<div>' + weeks + 'weekly [' + st + ' ~ ' + ed + ']</div>' + '</td>';
									}
						...
						if (!options.monthOnly) {
							...
							mounth_picker.find('.xdsoft_label').eq(0).css('width', '');
							mounth_picker.find('.xdsoft_label').eq(1).css('width', '');
						} else {
							mounth_picker.find('.xdsoft_label').eq(0).css('width', '0px');
							mounth_picker.find('.xdsoft_label').eq(1).css('width', '130px');
						}
						...
						var time = '',
							...
							line_time = function line_time(h, m) {
								...
								classes = [];
								if ((options.maxTime !== false && _xdsoft_datetime.strtotime(options.maxTime).getTime() / 10000 < parseInt(now.getTime() / 10000))) {
									classes.push('xdsoft_disabled');
									//console.info(_xdsoft_datetime.strtotime(options.maxTime) +"_"+now);
									//console.info("=============>>"+_xdsoft_datetime.strtotime(options.maxTime).getTime()/ 10000 + "___" + parseInt(now.getTime()/10000));
								}
								if ((options.minTime !== false && _xdsoft_datetime.strtotime(options.minTime).getTime() > now.getTime())) {
									classes.push('xdsoft_disabled');
								}
				...
				calendar
					.on('click.xdsoft', 'td', function() {
						...
						if (!options.usePartition) {
							...
							input.val(_xdsoft_datetime.str() + " ~ " + _xdsoft_datetime.apartDate(_xdsoft_datetime.currentTime));
						}
				...
				datetimepicker.mousewheel && timeboxparent.unmousewheel().mousewheel(function(event, delta, deltaX, deltaY) {
					...
					var pheight = timeboxparent[0].clientHeight,
						height = timebox[0].offsetHeight,
						top = Math.abs(parseInt(timebox.css('marginTop'))), fl = true;
				...
				var setPos = function() {
					...
					if (top + datetimepicker[0].offsetHeight > $(window).height() && (datetimepicker.data('input')[0].id == 'rule_search_start_picker' || datetimepicker.data('input')[0].id == 'rule_search_end_picker')){						
					...
					if(datetimepicker.data('input')[0].id == 'rule_search_mk_start_dt' || datetimepicker.data('input')[0].id == 'rule_search_mk_end_dt' || datetimepicker.data('input')[0].id == 'log_date' || datetimepicker.data('input')[0].id == 'tjd_date'){
						datetimepicker.css({
						'z-index': 11,
					});
					}
